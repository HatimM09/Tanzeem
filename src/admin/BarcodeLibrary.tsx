...
import {
  ...
  Trash2, FileDown, AlertTriangle, FileUp, X, Check,
} from 'lucide-react';
      ...
      <div style={{ padding: '0 0 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setShowImport(true)} className="btn-ghost" style={{ padding: '10px 16px', fontSize: 12 }}>
              <FileUp size={14} /> Import Items
            </button>
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ borderTop: '1px solid var(--border)', padding: 20, background: 'var(--bg-surface)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                      {[
                        ...
                        { icon: Calendar, label: 'Added Date', value: item.createdAt },
                      ].map(({ icon: Icon, label, value }) => (
                        <div key={label} style={{ background: 'var(--bg-card)', borderRadius: 12, padding: 12, border: '1px solid var(--border)' }}>
